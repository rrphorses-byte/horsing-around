...
const DEFAULT_RIDERS = [
  { id: 1, name: "Emma Thornton", age: 12, level: "Beginner", phone: "555-0101", email: "emma@example.com", balance: -45 },
  { id: 2, name: "James Whitfield", age: 16, level: "Intermediate", phone: "555-0102", email: "james@example.com", balance: 0 },
  { id: 3, name: "Sofia Reyes", age: 9, level: "Beginner", phone: "555-0103", email: "sofia@example.com", balance: -90 },
  { id: 4, name: "Luca Brennan", age: 14, level: "Advanced", phone: "555-0104", email: "luca@example.com", balance: 60 },
];
const DEFAULT_HORSES = [
  { id: 1, name: "Copper", breed: "Quarter Horse", age: 8, color: "#b5651d" },
  { id: 2, name: "Midnight", breed: "Friesian", age: 6, color: "#2c2c2c" },
  { id: 3, name: "Daisy", breed: "Appaloosa", age: 11, color: "#d4a96a" },
  { id: 4, name: "Storm", breed: "Andalusian", age: 9, color: "#8a9ba8" },
];
const DEFAULT_INSTRUCTORS = [
  { id: 1, name: "Sarah Mills", phone: "", email: "" },
  { id: 2, name: "Tom Harwick", phone: "", email: "" },
  { id: 3, name: "Lily Chen", phone: "", email: "" },
];
const DEFAULT_LESSON_TYPES = [
  { id: 1, name: "Private", duration: 60, price: 85 },
  { id: 2, name: "Group (2-4)", duration: 60, price: 45 },
  { id: 3, name: "Semi-Private", duration: 60, price: 65 },
  { id: 4, name: "Trail Ride", duration: 90, price: 55 },
];
const DEFAULT_LESSONS = [
  { id: 1, riderId: 1, horseId: 1, instructor: "Sarah Mills", type: "Private", date: "2026-03-17", time: "09:00", status: "Scheduled", paid: false, price: 85, notes: "" },
  { id: 2, riderId: 2, horseId: 2, instructor: "Tom Harwick", type: "Group (2-4)", date: "2026-03-17", time: "11:00", status: "Scheduled", paid: true, price: 45, notes: "" },
  { id: 3, riderId: 3, horseId: 3, instructor: "Lily Chen", type: "Semi-Private", date: "2026-03-18", time: "14:00", status: "Scheduled", paid: false, price: 65, notes: "" },
  { id: 4, riderId: 1, horseId: 1, instructor: "Sarah Mills", type: "Private", date: "2026-03-10", time: "09:00", status: "Completed", paid: true, price: 85, notes: "Great progress on posting trot. Emma is gaining confidence at the canter." },
  { id: 5, riderId: 4, horseId: 4, instructor: "Tom Harwick", type: "Private", date: "2026-03-11", time: "15:00", status: "Completed", paid: false, price: 85, notes: "Worked on lead changes. Luca needs more practice with left lead." },
];
const DEFAULT_PAYMENTS = [
  { id: 1, riderId: 2, lessonId: 2, amount: 45, date: "2026-03-15", method: "Card", note: "Lesson 3/17" },
  { id: 2, riderId: 1, lessonId: 4, amount: 85, date: "2026-03-10", method: "Cash", note: "Lesson 3/10" },
  { id: 3, riderId: 4, lessonId: null, amount: 60, date: "2026-03-12", method: "Venmo", note: "Account credit" },
];
...
export default function App() {
  ...
  const [lessons, setLessons] = useLocalStorage("ha_lessons", DEFAULT_LESSONS);
  const [riders, setRiders] = useLocalStorage("ha_riders", DEFAULT_RIDERS);
  const [horses, setHorses] = useLocalStorage("ha_horses", DEFAULT_HORSES);
  const [instructors, setInstructors] = useLocalStorage("ha_instructors", DEFAULT_INSTRUCTORS);
  const [lessonTypes, setLessonTypes] = useLocalStorage("ha_lesson_types", DEFAULT_LESSON_TYPES);
  const [payments, setPayments] = useLocalStorage("ha_payments", DEFAULT_PAYMENTS);
  ...
  const saveEdit = () => {
    ...
    if (type === "rider") {
      ...
      setRiders(riders.map(r => r.id === form.id ? { ...form, age: parseInt(form.age) || 0, balance: parseFloat(form.balance) || 0 } : r));
    } else if (type === "horse") {
      if (!form.name) { showToast("Name is required", "error"); return; }
      setHorses(horses.map(h => h.id === form.id ? { ...form, age: parseInt(form.age) || 0 } : h));
    } else if (type === "instructor") {
      if (!form.name) { showToast("Name is required", "error"); return; }
      setInstructors(instructors.map(i => i.id === form.id ? { ...form } : i));
    } else if (type === "lessonType") {
      if (!form.name) { showToast("Name is required", "error"); return; }
      setLessonTypes(lessonTypes.map(t => t.id === form.id ? { ...form, price: parseFloat(form.price) || 0, duration: parseInt(form.duration) || 60 } : t));
    }
  ...
  const deleteItem = (type, id) => {
    if (type === "rider") setRiders(riders.filter(r => r.id !== id));
    else if (type === "horse") setHorses(horses.filter(h => h.id !== id));
    else if (type === "instructor") setInstructors(instructors.filter(i => i.id !== id));
    else if (type === "lessonType") setLessonTypes(lessonTypes.filter(t => t.id !== id));
    setModal(null); setForm({}); setEditTarget(null);
    showToast("Deleted");
  };
  ...
  const addNew = (type) => {
    ...
    if (type === "rider") { setForm({ id: newId, name: "", age: "", level: "Beginner", phone: "", email: "", balance: 0 }); }
    else if (type === "horse") { setForm({ id: newId, name: "", breed: "", age: "", color: "#b5651d" }); }
    else if (type === "instructor") { setForm({ id: newId, name: "", phone: "", email: "" }); }
    else if (type === "lessonType") { setForm({ id: newId, name: "", duration: 60, price: 0 }); }
  ...
  const saveNew = () => {
    const { type } = editTarget;
    if (!form.name) { showToast("Name is required", "error"); return; }
    if (type === "rider") setRiders([...riders, { ...form, age: parseInt(form.age) || 0, balance: parseFloat(form.balance) || 0 }]);
    else if (type === "horse") setHorses([...horses, { ...form, age: parseInt(form.age) || 0 }]);
    else if (type === "instructor") setInstructors([...instructors, { ...form }]);
    else if (type === "lessonType") setLessonTypes([...lessonTypes, { ...form, price: parseFloat(form.price) || 0, duration: parseInt(form.duration) || 60 }]);
    setModal(null); setForm({}); setEditTarget(null);
    showToast("Added!");
  };
  ...
  // ── Lesson actions ──
  const saveNotes = () => {
    setLessons(lessons.map(l => l.id === selectedLesson.id ? { ...l, notes: tempNotes } : l));
  ...
  const handleAddLesson = () => {
    ...
    setLessons([...lessons, { id: Date.now(), riderId: parseInt(form.riderId), horseId: parseInt(form.horseId), instructor: form.instructor, type: form.type, date: form.date, time: form.time, status: "Scheduled", paid: false, price: type?.price || 0, notes: form.notes || "" }]);
    setSelectedDay(form.date); setModal(null); setForm({});
    showToast("Lesson scheduled!");
  };

  const handlePayment = (lesson) => {
    setPayments([...payments, { id: Date.now(), riderId: lesson.riderId, lessonId: lesson.id, amount: lesson.price, date: today, method: form.method || "Cash", note: `Lesson ${lesson.date}` }]);
    setLessons(lessons.map(l => l.id === lesson.id ? { ...l, paid: true } : l));
    if (selectedLesson?.id === lesson.id) setSelectedLesson({ ...selectedLesson, paid: true });
    setModal(null); setForm({});
    showToast(`Payment of $${lesson.price} recorded!`);
  };

  const completeLesson = (id) => { setLessons(lessons.map(l => l.id === id ? { ...l, status: "Completed" } : l)); if (selectedLesson?.id === id) setSelectedLesson({ ...selectedLesson, status: "Completed" }); showToast("Lesson marked complete"); };
  const cancelLesson = (id) => { setLessons(lessons.map(l => l.id === id ? { ...l, status: "Cancelled" } : l)); if (selectedLesson?.id === id) setSelectedLesson({ ...selectedLesson, status: "Cancelled" }); showToast("Lesson cancelled"); };

  // ── AI ──
  const generateNotes = async (lesson) => {
    const rider = getRider(lesson.riderId); const horse = getHorse(lesson.horseId);
    setAiLoading(true);
    const text = await callClaude([{ role: "user", content: `Generate brief, practical lesson notes (3-5 sentences) for a ${lesson.type} riding lesson. Rider: ${rider?.name}, Level: ${rider?.level}, Horse: ${horse?.name} (${horse?.breed}), Instructor: ${lesson.instructor}. Focus on realistic observations and next steps.` }], "You are an expert equestrian instructor. Write concise, professional lesson notes.");
  ...
  const editFields = () => {
    ...
    if (type === "horse") return [
      ["Name", "name", "text", "Horse name"],
      ["Breed", "breed", "text", "Breed"],
      ["Age", "age", "number", "Age"],
      ["Color", "color", "color", ""],
    ];
    ...
    if (type === "lessonType") return [
      ["Name", "name", "text", "Lesson type name"],
  ...
  const editTitle = () => {
    const t = editTarget?.type;
    const isNew = editTarget?.isNew;
    if (t === "rider") return isNew ? "Add Rider" : "Edit Rider";
    if (t === "horse") return isNew ? "Add Horse" : "Edit Horse";
    if (t === "instructor") return isNew ? "Add Instructor" : "Edit Instructor";
    if (t === "lessonType") return isNew ? "Add Lesson Type" : "Edit Lesson Type";
    return "Edit";
  };
                ...
                return (
                  <div key={l.id} className="lesson-row" style={{ borderLeftColor: statusColor(l.status) }} onClick={() => { setSelectedLesson(l); setTempNotes(l.notes||""); setEditingNotes(false); setModal("lessonDetail"); }}>
                    ...
                    <div style={{ marginTop: 8 }}>
                      <span className="horse-chip" style={{ background: `${horse.color}22`, color: horse.color==="#2c2c2c"?"#555":horse.color }}>🐎 {l.type}</span>
                    </div>
          ...
          <div>
            ...
            <div className="riders-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(290px, 1fr))", gap: 18 }}>
              {riders.map(r => {
                ...
                const avatarColor = ["#a0784a","#4a7c59","#2c6fa8","#9e5a3a"][r.id%4];
                return (
                  <div key={r.id} className="card" style={{ padding: 22 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
                      {[["Upcoming",upcoming,"#4a7c59"],["Completed",completed,"#2c6fa8"],["Balance",`$${Math.abs(r.balance)}`,r.balance<0?"#c8945a":"#4a7c59"]].map(([label,val,color])=>(
            ...
            <div className="settings-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              ...
              <div className="card" style={{ padding: 22 }}>
                <div className="section-header">
                  <div style={{ fontWeight: 600, fontSize: 17 }}>🐴 Horses</div>
                  <button className="btn" onClick={() => addNew("horse")} style={{ background: "#a0784a", color: "white", padding: "6px 14px", fontSize: 12 }}>+ Add</button>
                </div>
                {horses.map(h => (
                  <div key={h.id} className="edit-row">
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                      <div style={{ width: 28, height: 28, borderRadius: "50%", background: h.color, border: "2px solid white", boxShadow: "0 1px 4px rgba(0,0,0,0.2)", flexShrink: 0 }} />
                      <div>
                        <div style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: 14 }}>{h.name}</div>
                        <div style={{ fontFamily: "'Lato',sans-serif", fontSize: 12, color: "#8a6a4a" }}>{h.breed} · Age {h.age}</div>
                      </div>
                    </div>
                    <button className="btn" onClick={() => openEdit("horse", h)} style={{ background: "#e8d8bc", color: "#5a3a1a", padding: "5px 10px", fontSize: 12 }}>✏️ Edit</button>
                  </div>
                ))}
              </div>
              ...
              <div className="card" style={{ padding: 22 }}>
                <div className="section-header">
                  <div style={{ fontWeight: 600, fontSize: 17 }}>👤 Instructors</div>
                  <button className="btn" onClick={() => addNew("instructor")} style={{ background: "#a0784a", color: "white", padding: "6px 14px", fontSize: 12 }}>+ Add</button>
                </div>
                {instructors.map(inst => (
                  <div key={inst.id} className="edit-row">
                    <div>
                      <div style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: 14 }}>{inst.name}</div>
                      {inst.phone && <div style={{ fontFamily: "'Lato',sans-serif", fontSize: 12, color: "#8a6a4a" }}>{inst.phone}</div>}
                    </div>
                    <button className="btn" onClick={() => openEdit("instructor", inst)} style={{ background: "#e8d8bc", color: "#5a3a1a", padding: "5px 10px", fontSize: 12 }}>✏️ Edit</button>
                  </div>
                ))}
              </div>
              ...
              <div className="card" style={{ padding: 22 }}>
                <div className="section-header">
                  <div style={{ fontWeight: 600, fontSize: 17 }}>📋 Lesson Types</div>
                  <button className="btn" onClick={() => addNew("lessonType")} style={{ background: "#a0784a", color: "white", padding: "6px 14px", fontSize: 12 }}>+ Add</button>
                </div>
                {lessonTypes.map(t => (
                  <div key={t.id} className="edit-row">
                    <div>
                      <div style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: 14 }}>{t.name}</div>
                      <div style={{ fontFamily: "'Lato',sans-serif", fontSize: 12, color: "#8a6a4a" }}>{t.duration} min · ${t.price}</div>
                    </div>
                    <button className="btn" onClick={() => openEdit("lessonType", t)} style={{ background: "#e8d8bc", color: "#5a3a1a", padding: "5px 10px", fontSize: 12 }}>✏️ Edit</button>
                  </div>
                ))}
              </div>
              ...
              <div className="card" style={{ padding: 22 }}>
                <div className="section-header">
                  <div style={{ fontWeight: 600, fontSize: 17 }}>🧑 Riders</div>
                  <button className="btn" onClick={() => addNew("rider")} style={{ background: "#a0784a", color: "white", padding: "6px 14px", fontSize: 12 }}>+ Add</button>
                </div>
                {riders.map(r => (
                  <div key={r.id} className="edit-row">
                    <div>
                      <div style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: 14 }}>{r.name}</div>
                      <div style={{ fontFamily: "'Lato',sans-serif", fontSize: 12, color: "#8a6a4a" }}>{r.level} · Age {r.age}</div>
                    </div>
                    <button className="btn" onClick={() => openEdit("rider", r)} style={{ background: "#e8d8bc", color: "#5a3a1a", padding: "5px 10px", fontSize: 12 }}>✏️ Edit</button>
                  </div>
                ))}
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {!editingNotes && <>
                      <button className="ai-action-btn" style={{ fontSize:11 }} disabled={aiLoading} onClick={()=>generateNotes(l)}>{aiLoading?"Generating…":"✨ AI Generate"}</button>
                      <button className="btn" onClick={()=>{setTempNotes(l.notes||"");setEditingNotes(true);}} style={{ background:"#e8d8bc", color:"#5a3a1a", padding:"5px 12px", fontSize:12 }}>✏️ Edit</button>
                    </>}
                    {editingNotes && <>
                      <button className="btn" onClick={saveNotes} style={{ background:"#4a7c59", color:"white", padding:"5px 12px", fontSize:12 }}>✓ Save</button>
                      <button className="btn" onClick={()=>setEditingNotes(false)} style={{ background:"#e8d8bc", color:"#5a3a1a", padding:"5px 12px", fontSize:12 }}>Cancel</button>
                    </>}
                  </div>
                </div>
        ...
        <div className="modal-bg" onClick={() => { setModal(null); setForm({}); setEditTarget(null); }}>
          <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div key={key} style={{ marginBottom: 14 }}>
                ...
                {type === "select" ? (
                  ...
                  <div>
                    ...
                    <input className="input" type="text" value={form[key]||""} onChange={e=>setForm({...form,[key]:e.target.value})} placeholder="#hexcode" style={{ marginTop: 4 }} />
                  </div>
                ) : (
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button className="btn" onClick={editTarget.isNew ? saveNew : saveEdit} style={{ flex: 1, background: "#a0784a", color: "white", padding: "12px" }}>
                {editTarget.isNew ? "Add" : "Save Changes"}
              </button>
              {!editTarget.isNew && (
                <button className="btn" onClick={() => { if (window.confirm("Are you sure you want to delete this?")) deleteItem(editTarget.type, form.id); }} style={{ background: "#9e5a3a22", color: "#9e5a3a", padding: "12px 16px" }}>🗑 Delete</button>
              )}
        ...
        <div className="modal-bg" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea className="textarea" style={{ minHeight:70 }} placeholder="Any notes…" value={form.notes||""} onChange={e=>setForm({...form,notes:e.target.value})} />
            </div>
        ...
        <div className="modal-bg" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <select className="input" value={form.method||"Cash"} onChange={e=>setForm({...form,method:e.target.value})}>
                {["Cash","Card","Venmo","Zelle","Check"].map(m=><option key={m}>{m}</option>)}
              </select>
            </div>
